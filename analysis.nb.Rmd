...
use `separate()` and `str_remove()` to separate columns to two and remove ## in `text` data

```{r}
news <- news %>% tibble() 
  
news <- separate(news, c(document_id = 9), into = c("document_id", "text"), col = `.`,  remove = TRUE) 
...
## Option to use this method instead if it's cleaner
...
unique(news$document_id) %>% length()

```

sample creation
```{r}
# slice_sample(news, n=20) %>% 
#   str_split("\\s+") %>% length()
...
news_wlp$document_id %>%
  unique() %>% length()

news_wlp %>% head()

## testing news_wlp2 with new fill method
# news_wlp2 <- news_wlp2 %>%
#   mutate(document_id = word)

str_view(news_wlp2$word, "^##\\d") %>% print(n=10)

news_wlp2 <- news_wlp2 %>%
  mutate(document_id = ifelse(str_detect(word, "^##\\d{7}")==T, str_sub(word, 3,9), NA))

# news_wlp2 <- news_wlp2 %>%
#   mutate(document_id = ifelse(str_detect(word, "^(##\\d{7})|\\d{7}")==T, str_sub(word, 3,9), NA))
...
news_wlp %>% distinct(news_wlp$document_id) %>% str_detect("3032382") ## TRUE

news_wlp2 %>% distinct(news_wlp2$document_id) %>% str_detect("3000001") ## TRUE

news_wlp2 %>% distinct(news_wlp2$document_id) %>% str_detect("3058375") ## FALSE
news_wlp2 %>% distinct(news_wlp2$document_id) %>% str_detect("3032382") ## TRUE
...
- Need to str_subset so that it recognizes 

```{r}

pos_test <- c("JJ|FF")

test_set <- tibble(a = c("JJ", "JJ_T", "NNS", "NN_FF"), b = c(1, 2, 3, 4), c = c("a", "b", "c", "d"))

test_set %>% 
  filter(str_detect(a, pos_test))
...
vocab[!2 %in% stopword_list] %>% tail(50)
vocab[!term %in% stopword_list] %>% tail(50)
...
Stop words for TEXT data

- remove punctuation  
- normalize for capitalization  
- remove stop words  

Result should be a list of character vectors

```{r}
tokens %>% head()

## This worked
# tokens[[2]] %>% str_subset("[<p>\\.\\,]", negate=T) %>% str_to_lower()
# 
# str_to_lower(str_subset(tokens[[2]], "[<p>\\.\\,]", negate=T))
# 
# ## Make a recipe
# ~ str_to_lower(str_subset(.x, "[<p>\\.\\,]", negate=T))
# 
# ## Apply to full
# map(tokens, ~ str_to_lower(str_subset(.x, "[<p>\\.\\,]", negate=T)))
...
tokens_wlp <- map(tokens_wlp, ~ str_to_lower(str_subset(.x, stop_punctuation_regex, negate=T)))
...
tokens[[2]][! tokens %in% stop_words_snowball$word]

list5 <- list(c("hi","i","be","ben"), c("here", "be","the","salary","and"), c("another","north"))

list5[[2]] %>% str_subset(list5[[2]] %in% c("here","be", "and", "then", "sal"), negate=F)
list5[[2]] %>% str_detect(c("here","be", "and", "then", "sal"))
list5[[2]] %>% str_extract_all(c("here","be", "and", "then", "sal"))
str_detect(c("here","the"), string = list5[[2]]) %>% reduce(`|`) 

map(list5, ~ str_subset(.x, c("here","the"), negate=T)) 

c <- str_c(c("here","the"), collapse="|")

map(list5, ~ str_subset(.x, c, negate=T))

map(list5, ~ str_extract_all(.x, c))
...
```{r}
# Make table with snowball and "en" and my own punctuation lists
  
stop_words_snowball <- filter(stop_words, lexicon=="snowball") %>% select(word)

stopwords(kind = "en")

my_stopwords <- c("also", "even", "one", "yet")


stop_punctuation_sep <- c("<p>",",",".",":",";","*","@","'","\"","-","$","--",
"(",")","'s","'d","'t","n't","'ve","'re","'ll")

# stopword_list <- tibble(filter(stop_words, lexicon=="snowball")
stopword_list_pun_sep <- c(stop_words_snowball$word, stopwords(kind="en"), stop_punctuation_sep)
stopword_list_pun_com <- c(stop_words_snowball$word, stopwords(kind="en"), "[<p>]|[\\,\\.\\:\\;\\*\\@\\'\\\"\\-\\$\\(\\)]|(\\-\\-)|('s)|('d)|('t)|(n't)|('ve)|('re)|('ll)")

stop_word_list <- c(stopwords(kind="en"), stop_words_snowball$word)

# Is BE in the stopword lists? Check lemmas.
...
# Create iterator over tokens
# tokens <- space_tokenizer(news_wlp8mil$lemma)

tokens_wlp <- news_wlp8mil %>% select(document_id, lemma) %>% chop(lemma) %>% pull(lemma, document_id) #%>% 
  #as.list()
...
# Create vocabulary. Terms will be unigrams (simple words).
it <- itoken(tokens_wlp, progressbar = FALSE)
vocab <- create_vocabulary(it)
# Remove very frequent and very infrequent terms using min term count
...
Try DTM... (from pipeline code)

```{r}
dgcm <- create_dtm(it, vectorizer, type = "dgCMatrix")
dgtm <- create_dtm(it, vectorizer, type = "dgTMatrix")
dgrm <- create_dtm(it, vectorizer, type = "dgRMatrix")
csm <- create_dtm(it, vectorizer, type = "CsparseMatrix")
tsm <- create_dtm(it, vectorizer, type = "TsparseMatrix")
rsm <- create_dtm(it, vectorizer, type = "RsparseMatrix")
```
...
```{r}
glove <-  GlobalVectors$new(rank = 50, x_max = 10)
wv_main <-  glove$fit_transform(rsm, n_iter = 10, convergence_tol = 0.01, n_threads = 8)
wv_context = glove$components
word_vectors = wv_main + t(wv_context)
...
Term count min: Minimum number of token counts for a term to be included in the corpus. DONT FORGET TO SPECIFY TERM COUNT MAX (or use stop word list)

Use "snowball" lexicon for stop-words, then remove extra words that might be unnecessary
...
## Determine which max count you want to remove
# vocab %>% arrange(desc(term_count)) %>% head(100)

vocab <- prune_vocabulary(vocab, term_count_min = 2L, term_count_max = 170000L)
...
Run the WLP data to here and compare to the other pipeline
