...
tokens <- map(tokens, ~ str_to_lower(str_subset(.x, stop_punctuation, negate=T)))
...
tokens_wlp <- map(tokens_wlp, ~ str_to_lower(str_subset(.x, stop_punctuation, negate=T)))
...
# tokens_wlp <- news_wlp8mil %>% group_by(document_id) %>% list(news_wlp8mil$lemma) 
# lemma <- group_by(news_wlp8mil, document_id) %>% as.vector(news_wlp8mil$lemma) %>% head()
# tokens_wlp <- news_wlp8mil %>% group_by(document_id) %>% as.vector(news_wlp8mil$lemma) 
# news_wlp8mil %>% group_by(document_id) %>% news_wlp8mil$lemma
# tokens_wlp1 <- news_wlp8mil$lemma
