...
## Building a DTM (Use 2)

Source of demo: (text2vec 0.3)[https://dsnotes.com/text2vec-0-3/]
...
tokens2 <- news_wlp %>% select(document_id, lemma) %>% chop(lemma) %>% pull(lemma, document_id) #%>% 
  #as.list()

## Keep names for dtm

names(tokens2) <- unique(news_wlp$document_id)

stop_punctuation_regex <- "[<p>]|[\\,\\.\\:\\;\\*\\@\\'\\\"\\-\\$\\(\\)\\?]|(\\-\\-)|('s)|('d)|('t)|(n't)|('ve)|('re)|('ll)"

tokens2 <- map(tokens2, ~ str_to_lower(str_subset(.x, stop_punctuation_regex, negate=T)))
...
# Create vocabulary. Terms will be unigrams (simple words).
it2 <- itoken(tokens2, progressbar = FALSE)
...
**Add stopwords in argument in vocabulary function**??

```{r}
vocab2 <- create_vocabulary(it2, stopwords = stop_word_list)
...
## Determine which max count you want to remove if any
# vocab2 %>% arrange(desc(term_count)) %>% head(100)

# add argument term_count_max = 170000L)
vocab2 <- prune_vocabulary(vocab2, term_count_min = 2L, doc_proportion_max = 0.5)
str(vocab2) # should be list of 4: vocab, ngram, doc count, stopwords
```
...
dtm <- create_dtm(it2, vectorizer2)

identical(rownames(dtm), unique(news_wlp$document_id)) #TRUE

# dgcm <- create_dtm(it2, vectorizer2, type = "dgCMatrix")
...
wv_main2 <-  glove$fit_transform(dtm, n_iter = 10, convergence_tol = 0.01, n_threads = 8)

wv_main2 <-  glove$fit_transform(dtm, n_iter = 10, convergence_tol = 0.01)
wv_main2 <-  glove$fit_transform(dtm, n_iter = 10, n_threads = 8)
wv_main2 <-  glove$fit_transform(dtm, convergence_tol = 0.01, n_threads = 8)
