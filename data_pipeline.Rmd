...
```{r}
## Benchmark has the lowest number, so we will restrict to 15
...
tibble(~ seed_list_asc | cos_tbl_15$words)

cos_tbl_15 %>% 
  unnest_wider(target)

map(cos_tbl_15, ~ filter(cos_tbl_15$target == .x)) %>% pull(words))
...
dcast(as.data.table(cos_tbl_15), words ~ target)
